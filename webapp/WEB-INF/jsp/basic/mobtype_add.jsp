...
<html>
  ...
  <body>
    <my:navigation text="基础设置 >>　手机型号管理 >> 添加型号" percent="90"/>
    <form action="mobType_addType.do" method="post" >
      <table width="500" align="center" style="margin-top:30px" class="inputTable">       
        <tr>
          <th>型号编号：</th>
          <td><input class="text" name="mobType.mobtypeid"  value="${mobType.mobtypeid}" maxlength=10 pattern="/^[a-z]{2}-\d{3,7}$/i" info="必须以两位字母开头，后跟分隔线，之后3到7位数字！" /></td>
        </tr>    
        <tr>
          <th>品牌：</th>
          <td>
