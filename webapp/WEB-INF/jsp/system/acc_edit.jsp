...
<html>
  ...
  <body>
    <my:navigation text="系统管理 >>　帐号管理 >> 编辑帐号" percent="90"/>
    <form action="accManage_editAcc.do" method="post" onsubmit="">
      <table width="500" align="center" style="margin-top:30px" class="inputTable">
        <tr>
          <th>员工姓名：</th>
          <td>${acc.empName}
            <input type="hidden" name="acc.userId" value="${acc.userId}"/>
            <input type="hidden" name="acc.empName" value="${acc.empName}"/>
          </td>
        </tr>
        <tr>
          <th>帐号名称：</th>
          <td><input class="text" name="acc.account" value="${acc.account}"/><my:errInfo key="dupAcc"/></td>
        </tr>
        <tr>
          <th>密码：</th>
        ...
        <tr>
          <th>状态：</th>
          <td>
            <input type="radio" name="acc.status"  value="<%=SysUsers.STAT_ENABLE%>"
              <my:checked curVal="<%=SysUsers.STAT_ENABLE%>" compare="${acc.status}"/>
            />启用
